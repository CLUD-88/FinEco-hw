...
使用 OpenAI Codex 辅助检索 AKShare 文档、编写下载与分析代码、检查接口及数据质量、撰写初版解读。给 AI 的任务说明（根据实际对话整理，非逐字引述）：“请按 HW02-1 的要求使用 AKShare 获取 A 股历史行情与总市值，计算股票收益和两类组合，并核对日期、单位、缺失值及异常收益。”初稿核验发现腾讯行情的原始成交量在沪深股票间存在单位差异，因此主分析改用成交额；还检查了 10 只股票的共同交易日、总市值覆盖和 Notebook 执行错误。提交前本人仍需核验上述判断与结果。
""")
